...
def _get_best_results(hparams):
    ...
    tokens = []
    for metric in hparams.metrics:
        tokens.append("%s %.4f" % (metric, getattr(hparams,"best_" + metric) ))
    return ", ".join(tokens)
...
def _external_eval(model, global_step, sess, hparams, iterator,
    ...
    if decode:
        for metric in hparams.metrics:
            ...
            if save_on_best and scores[metric] > getattr(hparams, "top_score")[0]:
                new_top_score=[]
                new_top_score_name = []
                isTopScore = True
                for score, name in zip(getattr(hparams, "top_score"), getattr(hparams, "top_score_name")):
                ...
                setattr(hparams, "best_" + metric, new_top_score[len(new_top_score)-1])
                model.saver.save(
                    sess,
                    os.path.join(
                ...
                if new_top_score[0] != 0:
                    os.system('rm '+getattr(hparams, "best_" + metric + "_dir")+'/translate.ckpt-'+new_top_score_name[0]+'*')
