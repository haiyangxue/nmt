...
def create_hparams(flags):
    #print(flags.test_prefix+"AAAAAAAAAAAAA")
...
def extend_hparams(hparams):
    ...
    top_score=[0,0,0,0,0]
    top_score_name=['','','','','']
    hparams.add_hparam("top_score", top_score)
    hparams.add_hparam("top_score_name", top_score_name)
    return hparams
...
def create_or_load_hparams(out_dir, default_hparams, hparams_path):
    ...
    print(getattr(hparams, "top_score"))
    # Save HParams
    ...
    if flags.inference_input_file:
        ...
        if ref_file and tf.gfile.Exists(trans_file):
            for metric in hparams.metrics:
                ...
                utils.print_out("  %s: %.4f" % (metric, score))
    else:
        # Train
