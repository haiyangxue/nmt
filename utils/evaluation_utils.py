...
def _bleu(ref_file, trans_file, subword_option=None):
  ...
  with tf.gfile.GFile("./tmp/eval/valid/ref", "w") as ref_w:
    for words in per_segment_references:
      sen=''
      for word in words[0]:
        sen+=str(word)
      ref_w.write(sen+'\n')
  with tf.gfile.GFile("./tmp/eval/valid/res", "w") as res_w:
    for words in translations:
        sen = ''
        for word in words:
          sen += str(word)
        res_w.write(sen + '\n')
  result = os.popen("sh ./tmp/eval/tools/run.sh").read()
  print("bleu_score: "+result)
  bleu_score=float(result.split("=")[1])
